...
const srcCss = process.env.GLANCETHING_APPS_SRC
  ? path.resolve(process.env.GLANCETHING_APPS_SRC)
  : path.resolve(
      root,
      '..',
      'glancething',
...
const APPS = [
  ...
  },
  {
    ...
    credit: 'GlanceThing-native.',
    css: 'MicApp.module.css',
    builtin: true
  },
  {
    id: 'weather',
    ...
    icon: 'wb_sunny',
    color: '#38bdf8',
    description: 'Fullscreen 10-day forecast from the tray, plus a weather widget for home pages.',
    credit: 'GlanceThing-native. Weather, calendar, AI usage and photos are home features that also ship as tray apps.',
    css: 'WeatherApp.module.css',
    builtin: true
  },
  {
    id: 'calendar',
    ...
    icon: 'calendar_today',
    color: '#f59e0b',
    description: 'Today and tomorrow from Teams, Mac, Slack, or Google Calendar. Ships a calendar widget for the layout.',
    credit: 'GlanceThing-native.',
    css: 'CalendarApp.module.css',
    builtin: true
  },
  {
    id: 'usage',
    ...
    icon: 'data_usage',
    color: '#a78bfa',
    description: 'Codex, Claude and Cursor limits and spend, with widgets you drop on a page.',
    credit: 'GlanceThing-native. Follows the approach of CodexBar.',
    css: 'UsageApp.module.css',
    builtin: true
  },
  {
    id: 'photos',
    ...
    icon: 'photo_library',
    color: '#fb7185',
    description: 'Browse the photo album from the tray. Ships a photo widget for home pages.',
    credit: 'GlanceThing-native.',
    css: 'PhotosApp.module.css',
    builtin: true
  },
  {
    id: 'exampleapp',
...
`,
  ...
  mic: wsApp('mic', 'Mic', 'mic'),
  weather: wsApp('weather', 'Weather', 'layout'),
  calendar: wsApp('calendar', 'Calendar', 'layout'),
  usage: wsApp('usage', 'AI usage', 'layout'),
  photos: wsApp('photos', 'Photos', 'screensaver')
}
...
GT.connect(function (ws) {
  ...
  else if ('${type}' === 'mic') ws.sendType('mic', 'watch')
  else if ('${type}' === 'layout') ws.sendType('layout')
  else if ('${type}' === 'screensaver') ws.sendType('screensaver', 'getAlbum')
  ws.listen(function (msg) {
    if (msg.type !== '${type}' && msg.type !== 'playback' && msg.type !== 'layout' && msg.type !== 'screensaver') return
    state.data = msg.data
...
for (const app of APPS) {
  ...
  if (!JS[app.id]) {
    throw new Error('No client JS generator for ' + app.id)
  }
  ...
  const source = SOURCE[app.id]
  if (source) {
    const tsx = path.join(srcCss, source + '.tsx')
    const css = path.join(srcCss, source + '.module.css')
    if (fs.existsSync(tsx)) {
      fs.copyFileSync(tsx, path.join(dir, 'src', source + '.tsx'))
    }
    if (fs.existsSync(css)) {
      fs.copyFileSync(css, path.join(dir, 'src', source + '.module.css'))
    }
  }
  const gt = path.join(root, 'lib', 'gt.js')
  const base = path.join(root, 'lib', 'base.css')
  if (fs.existsSync(gt)) {
    fs.copyFileSync(gt, path.join(dir, 'client', 'gt.js'))
  }
  if (fs.existsSync(base)) {
    fs.copyFileSync(base, path.join(dir, 'client', 'base.css'))
  }
}
